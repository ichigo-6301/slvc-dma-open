...
| Memory and CQ | `dma_axi_write_engine`, `dma_axi_write_engine_512`, `dma_axi_write_engine_64_stream`, `dma_axi_read_prefetch`, `dma_cq_writer`, `dma_cq_single_writer` | Default RX write, optional 64/512-bit RX writers, TX read/prefetch, and owner-last completion publication. |
| RX memory CDC | `dma_rx_payload_cdc_bridge`, `dma_rx_payload_serializer_512_to_64`, `dma_async_fifo_tech`, `dma_reset_sync` | Tagged command/payload/completion crossing, memory-domain serialization, technology binding, and per-domain reset release. |
...
The catalog describes the frozen 512-bit source set plus optional adapter and
RX memory development sources. It is not a promise that
