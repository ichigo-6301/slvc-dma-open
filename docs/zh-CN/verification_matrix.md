...
| `run_rtl_rx_payload_cdc_bridge.do` | Command/payload/completion CDC、tag、FIFO 压力、6 种 clock profile 和 clock stop | `PASS tb_rtl_rx_payload_cdc_bridge frames=450 bytes=924873 source_stalls=321 fifo_empty=169 peak_payload_level=32 clock_profiles=6 clock_stops=2` |
...
| `run_rtl_rx_mem_async64_integration.do` | Async64 fixed/shared source 顺序与延迟 soft-reset drain | `PASS tb_rtl_rx_mem_async64_integration directed_lengths=18 mixed_frames=256 soft_reset_drain=1` |
...
| `run_rtl_rx_mem_async512_integration.do` | Async512 fixed/shared source 顺序与延迟 soft-reset drain | `PASS tb_rtl_rx_mem_async512_integration directed_lengths=18 mixed_frames=256 soft_reset_drain=1` |
...
wide marker，共 12 项。每个异步 defconfig 调度 10 项 core、1 项公共 CDC bridge
和对应 2 项 width-specific test，共 13 项。该矩阵不等价于 coverage closure、
formal proof 或完整 CDC/RDC signoff。
