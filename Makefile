...
POWER_RESEARCH_VALIDATOR = $(PYTHON) "$(ROOT)/flows/scripts/validate_asic_power_clock_gating_experiment.py" --root "$(ROOT)"
...
>   '  make power-research-check         Validate branch-only negative ASIC power research (no EDA)' \
...
> @cd "$(ROOT)" && $(PYTHON) -m unittest flows.scripts.test_validate_asic_power_clock_gating_experiment
